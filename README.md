...
The control can use two window handle values: *prev* and *next*, which are NULL by default. If they are not NULL, the Tab handler should send WM_NEXTDLGCTL message to the parent dialog with *next* value as wParam, if Tab was pressed, or with *prev* value as wParam, if Shift+Tab were pressed. lParam should have TRUE in the low word.
...
Also, WM_CHANGEUISTATE message should be sent to the parent dialog to update focus rect.
